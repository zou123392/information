...
export default {
  ...
  mounted () {
    if (this.$route.query.p) {
      this.p = JSON.parse(this.$route.query.p)
    }

    if (this.p) {
      this.title = '商品详情'
      this.ruleForm.name = this.p.name
      this.ruleForm.type = this.p.title
      this.ruleForm.price = this.p.price
      this.ruleForm.discount = this.p.discount
      this.ruleForm.img = this.p.img
      this.ruleForm.detailImg = this.p.detail_img
      this.ruleForm.status = this.p.status
      this.ruleForm.desc = this.p.desc
  ...
  methods: {
    ...
    validDiscount (rule, value, callback) {
      ...
      setTimeout(() => {
        let reg = /^\d(\.\d{1,2})?$|^10(\.0{1,2})?$/
        if (!reg.test(value)) {
          callback(new Error('折扣只能填写1~10之间的数值'))
        } else {
          callback()
        }
      }, 400)
